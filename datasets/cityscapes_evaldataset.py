...
class CityscapesEvalDataset(MonoDataset):
    ...
    def __init__(self, *args, image_data_path=None, camera_data_path=None, **kwargs):
        super(CityscapesEvalDataset, self).__init__(*args, **kwargs)
        # 图像根目录（可指向 Cityscapes-C 的 corruption/severity_x）
        self.image_data_path = image_data_path if image_data_path is not None else self.data_path
        # 相机内参根目录（通常应指向原始 Cityscapes 根目录）
        self.camera_data_path = camera_data_path if camera_data_path is not None else self.data_path
    ...
    def load_intrinsics(self, city, frame_name):
        ...
        camera_file = os.path.join(self.camera_data_path, 'camera_trainvaltest', 'camera',
    ...
    def get_offset_framename(self, frame_name, offset=0):
    ...
    def get_colors(self, city, frame_name, side, do_flip):
        ...
        prev_name = self.get_offset_framename(frame_name, offset=0)
    ...
    def get_image_path(self, city, frame_name, side, is_sequence=False):
        ...
        image_path = os.path.join(
            self.image_data_path, folder, split, city, frame_name + '_leftImg8bit.png')
        return image_path
