...
def main():
    ...
    parser.add_argument('--tta_mode', type=str, choices=['vec', 'eata', 'ssa'], default='vec', 
                       help='选择TTA模式: vec=VECTTA, eata=EATA, ssa=Regression SSA')
    parser.add_argument('--eval_data_path', type=str, required=True,
                       help='Cityscapes数据集路径')
    parser.add_argument('--log_dir', type=str, required=True)
    parser.add_argument('--model_name', type=str, required=True)
    parser.add_argument('--dataset', type=str, default='cityscapes')
    parser.add_argument('--split', type=str, help='数据集分割（用于兼容性，实际使用eval_split）')
    ...
    parser.add_argument('--eval_mono', action='store_true', help='单目评估模式')
    parser.add_argument('--save_pred_disps', action='store_true', help='保存预测的视差图')
    parser.add_argument('--eata_steps', type=int, default=1)
    parser.add_argument('--clean_pred_path', type=str, default='clean_pred_disps_cityscapes.npy', 
    ...
    # 使用 CityscapesEvalDataset 替代 KITTIRAWDataset
    ...
    # 先加载 encoder_dict 以获取保存的参数
    encoder_path = os.path.join(args.load_weights_folder, "encoder.pth")
    encoder_dict = torch.load(encoder_path)
    
    # 尝试从 checkpoint 中读取保存的参数
    saved_num_features = encoder_dict.get('num_features', None)
    saved_model_dim = encoder_dict.get('model_dim', None)
    saved_num_layers = encoder_dict.get('num_layers', None)
    
    # 如果 checkpoint 中没有保存，尝试从权重形状推断 num_features
    if saved_num_features is None and 'decoder.conv2.weight' in encoder_dict:
        # decoder.conv2.weight 的形状是 [num_features, bottleneck_features, 1, 1]
        # 对于 ResNet50，bottleneck_features = 2048
        conv2_weight = encoder_dict['decoder.conv2.weight']
        if conv2_weight.shape[1] == 2048:  # ResNet50
            saved_num_features = conv2_weight.shape[0]
            print(f"Inferred num_features from checkpoint: {saved_num_features}")
    
    # 使用保存的参数或命令行参数
    num_features = saved_num_features if saved_num_features is not None else args.num_features
    model_dim = saved_model_dim if saved_model_dim is not None else args.model_dim
    num_layers = saved_num_layers if saved_num_layers is not None else args.num_layers
    
    if saved_num_features is not None:
        print(f"Using num_features={num_features} from checkpoint")
    if saved_model_dim is not None:
        print(f"Using model_dim={model_dim} from checkpoint")
    if saved_num_layers is not None:
        print(f"Using num_layers={num_layers} from checkpoint")
    ...
    if args.backbone in ["resnet", "resnet_lite"]:
        ...
        encoder = networks.LiteResnetEncoderDecoder(model_dim=model_dim)
    elif args.backbone == "eff_b5":
        encoder = networks.BaseEncoder.build(num_features=num_features, model_dim=model_dim)
    else:
        encoder = networks.Unet(pretrained=False, backbone=args.backbone, in_channels=3, 
                                num_classes=model_dim, decoder_channels=[1024, 512, 256, 128])

    model_dict = encoder.state_dict()
    encoder.load_state_dict({k: v for k, v in encoder_dict.items() if k in model_dict})
    encoder.cuda().eval()

    # 从 decoder checkpoint 推断参数
    decoder_path = os.path.join(args.load_weights_folder, "depth.pth")
    decoder_dict = torch.load(decoder_path)
    
    # 尝试从 checkpoint 中读取保存的参数
    saved_dim_out = decoder_dict.get('dim_out', None)
    saved_query_nums = decoder_dict.get('query_nums', None)
    saved_patch_size = decoder_dict.get('patch_size', None)
    
    # 如果 checkpoint 中没有保存，尝试从权重形状推断
    if saved_dim_out is None:
        # 从 convert_to_prob.0.weight 推断 dim_out
        # convert_to_prob.0.weight 的形状是 [dim_out, query_nums, 1, 1]
        if 'convert_to_prob.0.weight' in decoder_dict:
            convert_weight = decoder_dict['convert_to_prob.0.weight']
            saved_dim_out = convert_weight.shape[0]
            print(f"Inferred dim_out from checkpoint: {saved_dim_out}")
            if saved_query_nums is None:
                saved_query_nums = convert_weight.shape[1]
                print(f"Inferred query_nums from checkpoint: {saved_query_nums}")
    
    # 从 bins_regressor 推断 embedding_dim * query_nums
    # bins_regressor.0.weight 的形状是 [16*query_nums, embedding_dim*query_nums]
    if 'bins_regressor.0.weight' in decoder_dict:
        bins_regressor_weight = decoder_dict['bins_regressor.0.weight']
        expected_input_dim = bins_regressor_weight.shape[1]  # embedding_dim * query_nums
        expected_output_dim = bins_regressor_weight.shape[0]  # 16 * query_nums
        
        # 如果 query_nums 已经推断出，可以计算 embedding_dim
        if saved_query_nums is not None:
            inferred_embedding_dim = expected_input_dim // saved_query_nums
            if inferred_embedding_dim * saved_query_nums == expected_input_dim:
                print(f"Inferred embedding_dim from bins_regressor: {inferred_embedding_dim}")
                # 如果推断出的 embedding_dim 与 model_dim 不匹配，需要调整
                if inferred_embedding_dim != model_dim:
                    print(f"Warning: embedding_dim mismatch! Checkpoint uses {inferred_embedding_dim}, but model_dim={model_dim}")
                    print(f"Using embedding_dim={inferred_embedding_dim} from checkpoint")
                    model_dim = inferred_embedding_dim
        else:
            # 如果 query_nums 未推断出，尝试从输出维度推断
            if expected_output_dim % 16 == 0:
                inferred_query_nums = expected_output_dim // 16
                if saved_query_nums is None:
                    saved_query_nums = inferred_query_nums
                    print(f"Inferred query_nums from bins_regressor: {inferred_query_nums}")
                inferred_embedding_dim = expected_input_dim // inferred_query_nums
                if inferred_embedding_dim * inferred_query_nums == expected_input_dim:
                    print(f"Inferred embedding_dim from bins_regressor: {inferred_embedding_dim}")
                    if inferred_embedding_dim != model_dim:
                        print(f"Warning: embedding_dim mismatch! Checkpoint uses {inferred_embedding_dim}, but model_dim={model_dim}")
                        print(f"Using embedding_dim={inferred_embedding_dim} from checkpoint")
    ...
    # 推断 decoder 类型：检查 transformer 的 dim_feedforward
    # Depth_Decoder_QueryTr 使用 dim_feedforward=1024
    # Lite_Depth_Decoder_QueryTr 使用 dim_feedforward=512
    use_lite_decoder = args.backbone.endswith("_lite")
    if 'transformer_encoder.layers.0.linear1.weight' in decoder_dict:
        transformer_dim = decoder_dict['transformer_encoder.layers.0.linear1.weight'].shape[0]
        if transformer_dim == 1024:
            use_lite_decoder = False
            print(f"Detected non-lite decoder (transformer dim_feedforward=1024)")
        elif transformer_dim == 512:
            use_lite_decoder = True
            print(f"Detected lite decoder (transformer dim_feedforward=512)")
        else:
            print(f"Warning: Unknown transformer dim_feedforward={transformer_dim}, using backbone setting")
    
    # 使用保存的参数或命令行参数
    dim_out = saved_dim_out if saved_dim_out is not None else args.dim_out
    query_nums = saved_query_nums if saved_query_nums is not None else args.query_nums
    patch_size = saved_patch_size if saved_patch_size is not None else args.patch_size
    
    # 确保 embedding_dim 与 model_dim 一致（decoder 的 embedding_dim 应该等于 encoder 的 model_dim）
    # 但如果从 bins_regressor 推断出了不同的值，使用推断的值
    embedding_dim = model_dim
    
    # 再次检查 bins_regressor 以确保 embedding_dim 正确
    if 'bins_regressor.0.weight' in decoder_dict and query_nums is not None:
        bins_regressor_weight = decoder_dict['bins_regressor.0.weight']
        expected_input_dim = bins_regressor_weight.shape[1]  # embedding_dim * query_nums
        inferred_embedding_dim = expected_input_dim // query_nums
        if inferred_embedding_dim * query_nums == expected_input_dim:
            if inferred_embedding_dim != model_dim:
                print(f"Warning: embedding_dim mismatch detected!")
                print(f"  Encoder model_dim: {model_dim}")
                print(f"  Decoder expects embedding_dim: {inferred_embedding_dim}")
                print(f"  Using decoder's expected embedding_dim={inferred_embedding_dim}")
                embedding_dim = inferred_embedding_dim
                # 注意：这里不能修改 model_dim，因为 encoder 已经创建了
                # 但我们需要确保 decoder 使用正确的 embedding_dim
    
    if saved_dim_out is not None:
        print(f"Using dim_out={dim_out} from checkpoint")
    if saved_query_nums is not None:
        print(f"Using query_nums={query_nums} from checkpoint")
    if saved_patch_size is not None:
        print(f"Using patch_size={patch_size} from checkpoint")
    print(f"Using embedding_dim={embedding_dim} for decoder")
    ...
    if use_lite_decoder:
        decoder = networks.Lite_Depth_Decoder_QueryTr(
            ...
            dim_out=dim_out,
            embedding_dim=embedding_dim,  # 使用推断出的 embedding_dim
    ...
    if args.mode == 'generate_clean':
        ...
        for batch in dataloader:
            ...
            if clean_pred_disps is not None:
                ...
                if args.post_process:
                    # post_process 模式下，batch_size 会翻倍
                    clean_depth_batch = clean_pred_disps[idx:idx+batch_size//2]
                    clean_depth_batch = np.concatenate([clean_depth_batch, clean_depth_batch], axis=0)
                    clean_depth = torch.from_numpy(clean_depth_batch).cuda()
                    idx += batch_size // 2
                else:
            ...
            # 处理 post_process（合并左右翻转的结果）
            if args.post_process:
                n = pred_disp.shape[0] // 2
                # 简单的后处理：平均左右翻转的结果
                pred_disp = 0.5 * (pred_disp[:n] + pred_disp[n:, :, ::-1])
        ...
        # === 评测 ===
        # Cityscapes 的 GT 深度是单独的文件，需要逐个加载
        ...
        print("-> Evaluating Cityscapes predictions...")
        for i in range(pred_disps.shape[0]):
            # 加载对应的 GT 深度
            gt_depth = np.load(os.path.join(gt_depths_dir, str(i).zfill(3) + '_depth.npy'))
            gt_height, gt_width = gt_depth.shape[:2]
            
            # crop ground truth to remove ego car -> this has happened in the dataloader for input images
            ...
            # Cityscapes 的特殊裁剪：centre crop to the middle 50% of the image
        ...
        # 保存误差结果
        if args.save_pred_disps:
            tag = "mono" if args.eval_mono else "stereo"
            error_path = os.path.join(
                args.load_weights_folder, "{}_{}_errors.npy".format(tag, args.eval_split))
            print("-> Saving errors to {}".format(error_path))
            np.save(error_path, np.array(errors))
