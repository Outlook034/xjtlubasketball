"""
Completion-TTA (Test-Time Adaptation) for Depth Estimation on Cityscapes

本脚本基于 `cityscapes_cotta.py`，将 CoTTA 替换为 Completion-TTA：
- 使用 completion_tta.py 中的 CompletionTTAModule
...
from completion_tta import CompletionTTAModule, create_sparse_depth, SQLDepthModel  # noqa: E402
...
def main():
    ...
    # Completion-TTA 参数（参考 completion_tta.py）
    ...
    parser.add_argument('--tta_w_sparse', type=float, default=1.0, help='稀疏深度一致性损失权重')
    ...
    parser.add_argument('--sparse_keep_ratio', type=float, default=0.1, help='稀疏深度采样比例')
    ...
    # 更新 args.height 和 args.width 为实际使用的尺寸（用于 CompletionTTAModule）
    args.height = HEIGHT
    args.width = WIDTH

    # 创建 Completion-TTA 适配器
    if not args.disable_tta:
        completion_tta = CompletionTTAModule(depth_model, args).cuda()
        print("-> Completion-TTA enabled with update_mode={}, steps={}, lr={}".format(
            args.tta_update_mode, args.tta_steps, args.tta_lr))
    else:
        completion_tta = None
        depth_model.eval()
        print("-> Completion-TTA disabled, using original model")

    # 加载 GT depths 用于生成稀疏深度和评估
    ...
    for batch in tqdm(dataloader, desc="Evaluating Cityscapes with Completion-TTA"):
        images = batch[("color", 0, 0)].cuda().float()
        bs = images.shape[0]

        # 加载当前批次的 GT depth（用于生成稀疏深度）
        batch_gt_list = []
        for b in range(bs):
            gt_path = os.path.join(gt_depths_dir, f"{idx + b:03d}_depth.npy")
        ...
        if args.disable_tta:
            ...
            # 生成稀疏深度
            # 注意：batch_gt 是原始 GT depth 尺寸，需要 resize 到输入图像尺寸
            batch_gt_resized = []
            for gt in batch_gt:
            ...
            # Completion-TTA 适应和预测
            pred_depth = completion_tta.adapt(images, sparse_depth)
    ...
    print("\nCompletion-TTA on SQLDepth (Cityscapes)")
