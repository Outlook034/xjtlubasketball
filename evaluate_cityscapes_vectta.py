...
# Import VecTTA from Vectta.py
# Note: VecTTA internally uses BackprojectDepth, Project3D, and transformation_from_parameters
# from layers module, which are automatically loaded when Vectta.py is imported
...
def evaluate(opt):
    ...
    if opt.ext_disp_to_eval is None:
        ...
        if opt.generate_clean:
            print("-> Generating clean predictions (no TTA)")
            model = SQLDepthModel(
                encoder.module if isinstance(encoder, torch.nn.DataParallel) else encoder,
                depth_decoder.module if isinstance(depth_decoder, torch.nn.DataParallel) else depth_decoder
            )
            model.cuda()
            model.eval()
            ...
            clean_pred_path = opt.clean_pred_path if opt.clean_pred_path else 'clean_pred_disps.npy'
            np.save(clean_pred_path, pred_disps)
            print(f"-> Clean predictions saved to {clean_pred_path}")
            print(f"-> Shape: {pred_disps.shape}")
            return

        # Create a combined model for TTA if needed
        if opt.use_tta:
            # Remove DataParallel wrapper for TTA
            encoder_single = encoder.module if isinstance(encoder, torch.nn.DataParallel) else encoder
            decoder_single = depth_decoder.module if isinstance(depth_decoder, torch.nn.DataParallel) else depth_decoder
            depth_model = SQLDepthModel(encoder_single, decoder_single)
            depth_model.cuda()
            
            # Initialize VecTTA
            adapt_model = VECTTA(
                depth_model=depth_model,
                update_mode=opt.vec_update_mode,
                optimizer_lr=opt.vec_lr,
                steps=opt.vec_steps,
                early_stop=opt.vec_early_stop,
                early_stop_patience=opt.vec_early_stop_patience,
                grad_clip=opt.vec_grad_clip,
            )
            adapt_model.cuda()
            print("-> VecTTA enabled with update_mode={}, steps={}, lr={}".format(
                opt.vec_update_mode, opt.vec_steps, opt.vec_lr))
            
            # Load clean predictions if provided
            clean_pred_disps = None
            if opt.clean_pred_path and os.path.exists(opt.clean_pred_path):
                clean_pred_disps = np.load(opt.clean_pred_path)
                print("-> Loaded clean predictions from {}".format(opt.clean_pred_path))
        else:
            adapt_model = None
            clean_pred_disps = None
        ...
        # Use torch.no_grad() only if not using TTA
        inference_context = torch.no_grad() if not opt.use_tta else torch.enable_grad()
        clean_idx = 0
        ...
        with inference_context:
            for i, data in tqdm.tqdm(enumerate(dataloader)):
                ...
                # Get K matrix for VecTTA if needed
                K = None
                if opt.use_tta:
                    # Try to get K from data, use scale 0
                    if ('K', 0) in data:
                        K = data[('K', 0)]
                        if torch.cuda.is_available():
                            K = K.cuda()
                    else:
                        # If K is not available, create a default one based on image size
                        batch_size = input_color.shape[0]
                        # Default intrinsics for Cityscapes (normalized)
                        fx = fy = 0.58  # Approximate normalized focal length
                        cx = 0.5
                        cy = 0.5
                        K = torch.zeros(batch_size, 4, 4, device=input_color.device)
                        K[:, 0, 0] = fx
                        K[:, 1, 1] = fy
                        K[:, 0, 2] = cx
                        K[:, 1, 2] = cy
                        ...
                        if i == 0:  # Only print warning once
                            print("Warning: K matrix not found in data, using default intrinsics")

                if opt.use_tta:
                    # Use VecTTA for adaptation
                    clean_depth = None
                    if clean_pred_disps is not None:
                        batch_size = input_color.shape[0]
                        clean_disp_batch = clean_pred_disps[clean_idx:clean_idx+batch_size]
                        # Ensure shape is [B, H, W] or [B, 1, H, W]
