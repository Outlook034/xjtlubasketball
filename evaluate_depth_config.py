...
# 导入 VecTTA 相关类
from Vectta import SQLDepthModel, VECTTA, EATA, configure_model_for_eata
...
def evaluate(opt):
    ...
    if opt.ext_disp_to_eval is None:
        ...
        # 创建 SQLDepthModel 用于 VecTTA（如果需要）
        adapt_model = None
        clean_pred_disps = None
        if use_vectta:
            print("-> Setting up VecTTA...")
            # 创建不包装 DataParallel 的模型用于 VecTTA
            if opt.backbone in ["resnet", "resnet_lite"]:
                encoder_single = networks.ResnetEncoderDecoder(num_layers=opt.num_layers, num_features=opt.num_features, model_dim=opt.model_dim)
            elif opt.backbone == "resnet18_lite":
                encoder_single = networks.LiteResnetEncoderDecoder(model_dim=opt.model_dim)
            elif opt.backbone == "eff_b5":
                encoder_single = networks.BaseEncoder.build(num_features=opt.num_features, model_dim=opt.model_dim)
            else: 
                encoder_single = networks.Unet(pretrained=(not opt.load_pretrained_model), backbone=opt.backbone, in_channels=3, num_classes=opt.model_dim, decoder_channels=opt.dec_channels)
            ...
            if opt.backbone.endswith("_lite"):
                depth_decoder_single = networks.Lite_Depth_Decoder_QueryTr(in_channels=opt.model_dim, patch_size=opt.patch_size, dim_out=opt.dim_out, embedding_dim=opt.model_dim, 
                                                                    query_nums=opt.query_nums, num_heads=4, min_val=opt.min_depth, max_val=opt.max_depth)
            else:
