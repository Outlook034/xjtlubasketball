"""
Completion-TTA Fast (Adaptation Layer Only) for Depth Estimation on Cityscapes

- Independent entry script (does NOT modify existing scripts)
- Mirrors cityscapes_completiontta.py pipeline
- Uses CompletionTTAFastModule (only updates an adaptation layer)
- Evaluation is aligned with official baseline: uses disp_to_depth before metrics

GT depth is used only to sample sparse depth (same as your current completion script).
"""
...
# Import Completion fast module
from completion_tta import create_sparse_depth, SQLDepthModel
from completion_tta_fast import CompletionTTAFastModule
...
def main():
    ...
    # TTA params (same names as completion script)
    ...
    if not os.path.exists(encoder_path) or not os.path.exists(decoder_path):
        raise FileNotFoundError('encoder.pth or depth.pth not found in load_weights_folder')
    ...
    # Set args height/width for transforms
    args.height = HEIGHT
    args.width = WIDTH

    if not args.disable_tta:
        tta = CompletionTTAFastModule(depth_model, args).cuda()
        print(f'-> Completion-TTA Fast enabled: steps={args.tta_steps}, lr={args.tta_lr}')
    else:
        tta = None
        depth_model.eval()
    ...
    for batch in tqdm(dataloader, desc='Evaluating Cityscapes with Completion-TTA Fast'):
        images = batch[("color", 0, 0)].cuda().float()
        bs = images.shape[0]

        # Load GT for sparse sampling
        batch_gt_list = []
        for b in range(bs):
        ...
        if args.disable_tta:
            with torch.no_grad():
                ...
                if isinstance(disp, dict):
                    disp = disp[("disp", 0)]
                pred_depth, _ = disp_to_depth(disp, args.min_depth, args.max_depth)
        else:
    ...
    print('\nCompletion-TTA Fast on SQLDepth (Cityscapes)')
