...
# 2. 位姿模型 - 使用与trainer.py相同的PoseCNN
class PoseModel(nn.Module):
    def __init__(self, pose_cnn):
        super().__init__()
        self.pose_cnn = pose_cnn
    def forward(self, x1, x2):
        # 将两帧图像拼接为一个6通道输入
        x = torch.cat([x1, x2], dim=1)
        # 使用PoseCNN网络
        axisangle, translation = self.pose_cnn(x)
        # 将axisangle和translation合并为6维位姿向量
        # PoseCNN输出: axisangle [B, 1, 1, 3], translation [B, 1, 1, 3]
        pose = torch.cat([translation.squeeze(-1), axisangle.squeeze(-1)], dim=-1)
        return pose  # 返回相对位姿 [B, 6] (平移+旋转)
...
def configure_model_for_ss_tta(depth_model, pose_model):
    ...
    # 解冻位姿网络的后期层
    for param in pose_model.pose_cnn.parameters():
        param.requires_grad = True
...
def main():
    ...
    parser.add_argument('--ss_tta_steps', type=int, default=1, help='自监督TTA步数')
    ...
    # === 加载位姿网络 ===
    # 使用与trainer.py中相同的PoseCNN网络
    # 根据trainer.py中的配置：networks.PoseCNN(num_input_frames=2)
    pose_model = networks.PoseCNN(num_input_frames=2)
    
    # 加载位姿网络权重（如果提供）
    if args.pose_weights_folder is not None:
        pose_path = os.path.join(args.pose_weights_folder, "pose.pth")
        if os.path.exists(pose_path):
            state_dict = torch.load(pose_path)
            # 处理可能的DataParallel前缀
            state_dict = {k.replace("module.", ""): v for k, v in state_dict.items()}
            pose_model.load_state_dict(state_dict)
    
    pose_model = PoseModel(pose_model)
    pose_model.cuda()
            ...
            for offset in [-1, 1]:
                try:
                    ...
                    if offset == -1:
                        ...
                        if prev_frame >= 0:  # 确保不是负数
                            prev_image_path = os.path.join(
                                args.data_path, folder, 
                                f"image_0{dataset.side_map[side]}/data/{prev_frame:010d}{dataset.img_ext}"
                            )
                            if os.path.exists(prev_image_path):
                                prev_image = dataset.loader(prev_image_path)
                                prev_image = dataset.resize[0](prev_image)
                                prev_image = dataset.to_tensor(prev_image)
                                source_frames.append(prev_image.unsqueeze(0).cuda())
                    else:  # offset == 1
                        next_frame = frame_index + 1
                        next_image_path = os.path.join(
                            args.data_path, folder, 
                            f"image_0{dataset.side_map[side]}/data/{next_frame:010d}{dataset.img_ext}"
                        )
                        if os.path.exists(next_image_path):
                            next_image = dataset.loader(next_image_path)
                            next_image = dataset.resize[0](next_image)
                            next_image = dataset.to_tensor(next_image)
                            source_frames.append(next_image.unsqueeze(0).cuda())
                except:
                    pass  # 如果加载失败，跳过这一帧
            
            # 获取相机内参
            intrinsics = batch["K"].cuda() if "K" in batch else None
